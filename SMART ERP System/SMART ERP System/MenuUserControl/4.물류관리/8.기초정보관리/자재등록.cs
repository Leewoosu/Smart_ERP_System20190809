...
namespace SMART_ERP_System.MenuUserControl
{
    public partial class 자재등록 : UserControl
    {
        ...
        {
            //if (DB.공급업체.Get공급업체번호From이름(txb공급업체.Text).Count == 0)
            //{
            ...
            productInfo.무게 = int.Parse(txb무게.Text);
            productInfo.상도 = int.Parse(txb상도.Text);
            productInfo.하도 = int.Parse(txb하도.Text);
            productInfo.품목군 = txb품목군.Text;
            ...
            //공급자재리스트.공급업체번호 = DB.공급업체.Get공급업체번호From이름(txb공급업체.Text).Select(x => x.공급업체번호).First();

            DB.공급자재리스트.Insert(공급자재리스트);
        }
        private void Btn변경_Click(object sender, EventArgs e)
        {
            //string 공급업체번호 = DB.공급업체.Get공급업체번호From이름(txb공급업체.Text).Select(x => x.공급업체번호).First();
        ...
        private void Dgv자재_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            cbb검사여부.Text = DB.자재.Search검사여부(자재번호);
            txb무게.Text = DB.자재.Search무게(자재번호);
            txb상도.Text = DB.자재.Search상도(자재번호);
            txb하도.Text = DB.자재.Search하도(자재번호);
