...
namespace SMART_ERP_System.MenuUserControl
{
    public partial class 계정과목등록 : UserControl
    {
        ...
        private void TreeView_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                expandedNode = new List<string>();
                expandedNode = FormHelper.collectExpandedNodes(treeView.Nodes);

                treeView.Nodes.Clear();
                treeView.SetMenuItemsByAccount(out MenuItems);

                if (expandedNode.Count > 0)
                {
                    TreeNode IamExpandedNode;
                    for (int i = 0; i < expandedNode.Count; i++)
                    {
                        IamExpandedNode = FormHelper.FindNodeByName(treeView.Nodes, expandedNode[i]);
                        FormHelper.expandNodePath(IamExpandedNode);
                    }
                }
            }
        }
        ...
        private void Btn세목추가_Click(object sender, EventArgs e)
        {
            ...
            if (DB.계정과목그룹.IsExist(treeView.SelectedNode.Name) == false)
            {
                ...
                계정과목그룹.그룹명 = DB.계정과목.SearchAccountName(treeView.SelectedNode.Name);
