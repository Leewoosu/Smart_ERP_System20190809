...
{
    public partial class 매입마감 : UserControl
    {
        public 매입마감()
        {
            InitializeComponent();
        ...
        private void 매입마감_Load(object sender, EventArgs e)
        {
        ...
        {
            dgv자재상세전표.DataSource = null;
            dgv자재전표.DataSource = DB.자재전표.자재전표조회(txb거래처.Text);

            for (int i = 0; i < dgv자재전표.RowCount; i++)
                dgv자재전표.Rows[i].Cells[2].Value = DB.일반거래처.Get공급업체이름From번호(dgv자재전표.Rows[i].Cells[2].Value.ToString());

        }

        private void DataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            dgv자재상세전표.DataSource = DB.자재상세전표.Search자재상세전표(dgv자재전표.CurrentRow.Cells[0].Value.ToString());

            for (int i = 0; i < dgv자재상세전표.RowCount; i++)
                dgv자재상세전표.Rows[i].Cells[0].Value = DB.자재.Get자재이름(dgv자재상세전표.Rows[i].Cells[0].Value.ToString());
